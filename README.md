# SPOT インスタンスを使用する EC2 Auto Scaling の環境を作成してみよう
...
1. **asg-instance** という名前のインスタンスの実行中のステータスになっていることを確認します。
...
1. **asg-instance** という名前のインスタンスのうち、どれか １つのチェックボックスをチェックします。
1. ページ下部の **詳細** で **ライフサイクル** が **spot** であることを確認します。
